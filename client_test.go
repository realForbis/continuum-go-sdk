...
import (
	...
	"github.com/realForbis/continuum-go-sdk/pkg/random"
...
func TestClient_GenerateBlock(t *testing.T) {
	t.Skip()
	c, err := NewClient("ws://127.0.0.1:29736")
	//client, err := NewClient("http://47.244.138.61:9735")
	if err != nil {
		t.Fatal(err)
	}
	defer c.Close()
...
func TestClient_BlockConfirmedStatus(t *testing.T) {
	t.Skip()
	c, err := NewClient("http://47.244.138.61:9735")
	if err != nil {
		t.Fatal(err)
	}
...
func TestNewClient_Subscribe(t *testing.T) {
	t.Skip()
	c, err := NewClient("ws://127.0.0.1:29736")
	if err != nil {
		t.Fatal(err)
